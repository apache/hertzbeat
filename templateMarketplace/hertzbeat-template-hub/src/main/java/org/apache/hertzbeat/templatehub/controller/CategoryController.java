...
public class CategoryController {
    ...
    public ResponseEntity<Message<String>> addCategory(@PathVariable("name") String name,
        ...
        if (name.isEmpty() || description.isEmpty()) {
            throw new HertzbeatTemplateHubException("empty name/description");
//            return ResponseEntity.ok(Message.fail(FAIL_CODE,"id is empty"));
    ...
    public ResponseEntity<Message<String>> modifyCategory(@PathVariable("id") int id,
        ...
        if (id<=0 || name.isEmpty() || description.isEmpty()) {
            throw new HertzbeatTemplateHubException("empty id/name/description");
//            return ResponseEntity.ok(Message.fail(FAIL_CODE,"id is empty"));
        }
    ...
    public ResponseEntity<Message<String>> deleteCategory(@PathVariable("id") int id) {
        ...
        if (id<=0) {
            throw new HertzbeatTemplateHubException("empty id");
//            return ResponseEntity.ok(Message.fail(FAIL_CODE,"id is empty"));
        }
    ...
    public ResponseEntity<Message<List<Category>>> getAllCategory(@PathVariable("isDel") int isDel) {
        ...
        if (isDel!=0&&isDel!=1) {
            throw new HertzbeatTemplateHubException("error option");
//            return ResponseEntity.ok(Message.fail(FAIL_CODE,"id is empty"));
        }
    ...
    public ResponseEntity<Message<Page<Category>>> getCategoryPageByIsDel(@PathVariable("isDel") int isDel, @RequestParam int page, @RequestParam int size) {
        ...
        if (isDel!=0&&isDel!=1||page<0||size<0) {
            throw new HertzbeatTemplateHubException("Params Error");
//            return ResponseEntity.ok(Message.fail(FAIL_CODE,"id is empty"));
        }
