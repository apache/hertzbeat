...
public class VersionController {
    ...
    public ResponseEntity<Message<Page<Version>>> getVersionPageByTemplate(@PathVariable("template") int templateId, @RequestParam int page, @RequestParam int size){

        if(templateId==0|| page <0|| size <=0){
            return ResponseEntity.ok(Message.fail(FAIL_CODE,"Params Error"));
        }
        Page<Version> versionPageByTemplate = versionService.getVersionPageByTemplate(templateId, page, size);
    ...
    public ResponseEntity<Message<String>> starVersion(@RequestParam("user") int userId,@RequestParam("template") int templateId, @RequestParam("version") int versionId){
        ...
        if(isOk==0){
            throw new HertzbeatTemplateHubException("star version error");
