...
--
-- Current V200 schema baseline for provisioning an empty PostgreSQL target.
-- Historical databases apply the same-version V200 migration instead.
    ...
    create table hzb_alert_group (
        ...
        primary key (id),
        constraint unique_group_key unique (workspace_id, group_key)
    );
    ...
    create table hzb_alert_single (
        ...
        workspace_id varchar(128) default 'default' not null,
        primary key (id),
        constraint unique_fingerprint unique (workspace_id, fingerprint)
    );
...
CREATE UNIQUE INDEX uk_hzb_monitor_metric_layout_creator_app
