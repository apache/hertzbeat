...
CREATE
ALIAS RENAME_HOST_TO_INSTANCE AS $$
void renameHostToInstance(java.sql.Connection conn) throws java.sql.SQLException {
    boolean instanceExists = false;
    boolean
hostExists = false;
    try
(java.sql.ResultSet rs = conn.getMetaData().getColumns(null, null, "HZB_MONITOR", "INSTANCE")) {
    ...
    try (java.sql.Statement stmt = conn.createStatement()) {
        if (instanceExists) {
            if (hostExists) {
                ...
                stmt.execute
("ALTER TABLE HZB_MONITOR DROP COLUMN host");
}
        } else {
            if (hostExists) {
...
DROP
ALIAS RENAME_HOST_TO_INSTANCE;

-- Update instance with port
UPDATE HZB_MONITOR m
SET instance = CONCAT(instance, ':',
                      (SELECT param_value FROM HZB_PARAM p WHERE p.monitor_id = m.id AND p.field = 'port'))
WHERE EXISTS (SELECT 1 FROM HZB_PARAM p WHERE p.monitor_id = m.id AND p.field = 'port');

-- Migrate history table
CREATE
ALIAS MIGRATE_HISTORY_TABLE AS $$
void migrateHistoryTable(java.sql.Connection conn) throws java.sql.SQLException {
    boolean monitorIdExists = false;
    boolean
metricLabelsExists = false;
    try
(java.sql.ResultSet rs = conn.getMetaData().getColumns(null, null, "HZB_HISTORY", "MONITOR_ID")) {
    ...
    if (monitorIdExists) {
        try (java.sql.Statement stmt = conn.createStatement()) {
            if (!metricLabelsExists) {
                ...
                stmt.execute
("ALTER TABLE HZB_HISTORY ALTER COLUMN metric_labels SET DATA TYPE VARCHAR(5000)");
                stmt.execute
...
("UPDATE HZB_HISTORY SET instance = NULL");
                stmt.execute
("ALTER TABLE HZB_HISTORY ALTER COLUMN instance SET DATA TYPE VARCHAR(255)");
}
