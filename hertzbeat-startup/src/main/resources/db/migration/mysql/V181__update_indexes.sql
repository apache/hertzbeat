...
BEGIN
    ...
    IF table_exists = 1 THEN
        ...
        IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.STATISTICS
                  ...
                  AND INDEX_NAME = 'index_incident_component') THEN
            SET @drop_sql = 'DROP INDEX index_incident_component ON hzb_status_page_incident_component_bind';
            PREPARE stmt FROM @drop_sql;
            EXECUTE stmt;
            DEALLOCATE PREPARE stmt;
        END IF;
        ...
        IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.STATISTICS
                      WHERE TABLE_SCHEMA = DATABASE()
                      AND TABLE_NAME = 'hzb_status_page_incident_component_bind'
                      AND INDEX_NAME = 'idx_incident_component_incident_id') THEN
        ...
        IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.STATISTICS
                      WHERE TABLE_SCHEMA = DATABASE()
                      AND TABLE_NAME = 'hzb_status_page_incident_component_bind'
                      AND INDEX_NAME = 'idx_incident_component_component_id') THEN
