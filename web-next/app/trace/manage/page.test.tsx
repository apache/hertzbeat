...
describe('trace manage page', () => {
  ...
  it('opens trace details in a side waterfall modal instead of the inline route-level demo span band', () => {
    ...
    expect(source).toContain("const prefix = scope === 'span' ? 'data-trace-manage-drawer-span-attribute' : 'data-trace-manage-drawer-resource';");
    expect(source).toContain('`${prefix}-operator-action`');
    expect(source).toContain('`${prefix}-operator-owner`');
    expect(source).toContain('traceDrawerAttributeOperatorDataAttributes(scope, option.value as TraceAttributeOperator, name, value)');
