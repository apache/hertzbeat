...
describe('otlp page', () => {
  ...
  it('renders the HertzBeat-native intake cortex instead of a pixel-level external catalog copy', async () => {
    ...
    expect(html).toContain('relative overflow-hidden bg-[#0b0c0e] px-5 py-4');
    expect(html).toContain('mx-auto flex max-w-full flex-wrap items-center justify-center gap-3');
    expect(html).toContain('hidden h-px w-12 flex-none bg-[#252b34] sm:block md:w-24 xl:w-40');
    ...
    expect(html).toContain('data-otlp-center-source-card-a11y-label="Java"');
    expect(html).toContain(`aria-label="Java: ${tZh('otlp.source.item.java.description')}"`);
    expect(html).toContain(
      `aria-label="${tZh('otlp.source.item.cloudflare-logs.label')}: ${tZh('otlp.source.item.cloudflare-logs.description')}"`
    );
    expect(html).not.toContain('aria-label="JavaJava');
    expect(html).not.toContain('aria-label="CFCloudflare');
    ...
    expect(html).toContain('aria-hidden="true" data-otlp-center-brand-logo="java"');
    expect((html.match(/data-otlp-center-brand-mark="text-monogram"/g) ?? []).length).toBeGreaterThanOrEqual(8);
    expect(html).not.toContain('data-otlp-center-brand-art=');
    expect(html).toContain('/entities/7');
    expect(html).toContain('href="/setting/settings/token?scope=otlp-ingest&amp;source=otlp-intake"');
