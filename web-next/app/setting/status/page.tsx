...
import { createStatusPageComponent, createStatusPageIncident, deleteStatusPageComponent, deleteStatusPageIncident, loadStatusManagementData, saveStatusPageOrg, updateStatusPageComponent, updateStatusPageIncident } from '@/lib/setting-status/controller';
...
export default function SettingStatusPage() {
  ...
  const load = useCallback(async (): Promise<StatusManagementData> => {
    void refreshTick;
    return loadStatusManagementData(apiMessageGet, {
      search: incidentSearch,
      pageIndex: incidentPageIndex,
      pageSize: incidentPageSize
    });
  }, [incidentPageIndex, incidentPageSize, incidentSearch, refreshTick]);
  ...
  return (
    <ClientWorkbench load={load} loadingCopy={t('setting.status.loading')}>
      {data => {
        ...
        async function handleSaveOrg() {
          ...
          try {
            ...
            setOrgMessage(t('common.save-success'));
            setEditingOrg(false);
            setRefreshTick(value => value + 1);
          } catch (error) {
            setOrgError(error instanceof Error ? error.message : t('common.save-failed'));
          } finally {
            setSavingOrg(false);
          }
        }
        ...
        async function handleSaveComponent() {
          ...
          setComponentError(null);
          try {
            const payload = buildStatusComponentPayload({ ...componentDraft, orgId: data.org.id || componentDraft.orgId });
            ...
            setComponentMessage(t('common.save-success'));
            setEditingComponent(false);
            setRefreshTick(value => value + 1);
          } catch (error) {
            setComponentError(error instanceof Error ? error.message : t('common.save-failed'));
          } finally {
        ...
        async function handleDeleteComponent(componentOverride?: StatusPageComponent) {
          ...
          try {
            ...
            setComponentMessage(t('common.delete-success'));
            setComponentError(null);
            setEditingComponent(false);
            setRefreshTick(value => value + 1);
          } catch (error) {
            setComponentError(error instanceof Error ? error.message : t('common.delete-failed'));
          }
        }
        ...
        async function handleSaveIncident() {
          ...
          setIncidentError(null);
          try {
            const payload = buildStatusIncidentPayload({ ...incidentDraft, orgId: data.org.id || incidentDraft.orgId }, data.components);
            if (incidentDraft.id) {
              await updateStatusPageIncident(apiMessagePut as any, payload);
            } else {
              await createStatusPageIncident(apiMessagePost as any, payload);
            }
            setIncidentMessage(t('common.save-success'));
            setEditingIncident(false);
            setRefreshTick(value => value + 1);
          } catch (error) {
            setIncidentError(error instanceof Error ? error.message : t('common.save-failed'));
          } finally {
        ...
        async function handleDeleteIncident(incidentOverride?: StatusPageIncident) {
          ...
          try {
            ...
            setIncidentMessage(t('common.delete-success'));
            setIncidentError(null);
            setEditingIncident(false);
            setRefreshTick(value => value + 1);
          } catch (error) {
            setIncidentError(error instanceof Error ? error.message : t('common.delete-failed'));
          }
        }
