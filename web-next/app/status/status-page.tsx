...
} from '../../lib/status-center/controller';
import {
...
const PUBLIC_STATUS_SETTLED_CACHE_TTL_MS = 10_000;
...
function StatusPublicSurface({ data }: { data: StatusPageData }) {
  ...
  useEffect(() => {
    ...
    loadStatusPageIncidentFeed(apiMessageGet, {
      ...
      .catch(error => {
        if (!active) return;
        setIncidentError(describeStatusIncidentLoadFailure(error, t('status.public.incidents.load-failed')));
      })
      .finally(() => {
        if (active) {
          setIncidentLoading(false);
...
export default function StatusPage() {
  const { t } = useI18n();
  const publicStatusCurrentYear = useMemo(() => new Date().getFullYear(), []);
  const publicStatusIncidentListUrl = useMemo(
    () =>
      buildStatusIncidentListUrl({
        ...buildStatusIncidentYearQuery(publicStatusCurrentYear),
        pageIndex: 0,
        pageSize: 9999
      }),
    [publicStatusCurrentYear]
  );
  const publicStatusCacheKey = useMemo(
    () =>
      [
        'public-status',
        '/status/page/public/org',
        '/status/page/public/component',
        publicStatusIncidentListUrl
      ].join(':'),
    [publicStatusIncidentListUrl]
  );
  ...
  return (
    <ClientWorkbench
      load={load}
      loadingCopy={t('status.loading')}
      cacheKey={publicStatusCacheKey}
      cacheSettledTtlMs={PUBLIC_STATUS_SETTLED_CACHE_TTL_MS}
    >
