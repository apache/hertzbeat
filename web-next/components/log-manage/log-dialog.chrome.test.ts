...
describe('log dialog shared chrome', () => {
  ...
  it('adopts shared ops tokens across shared log dialog and row-list owners', () => {
    ...
    expect(streamDialogSource).not.toContain('归因诊断');
    expect(streamDialogSource).not.toContain('已提供');
    expect(streamDialogSource).not.toContain('缺失');
    ...
    expect(relatedTraceDialogSource).not.toContain('相关链路');
    expect(relatedTraceDialogSource).not.toContain('跨度事件');
    expect(relatedTraceDialogSource).not.toContain('事件详情');
    expect(relatedTraceDialogSource).not.toContain('查看跨度');
