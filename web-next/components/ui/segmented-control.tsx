...
export function SegmentedControl({ name, value, options, onChange, className, ...props }: SegmentedControlProps) {
  return (
    <span
      data-cold-segmented-control-owner="cold-segmented-control"
      role="radiogroup"
      className={cn('inline-flex min-h-8 overflow-hidden rounded-[3px] border border-[#2b3039] bg-[#101217] p-0.5', className)}
      {...props}
    >
      {name ? <HiddenInput name={name} value={value} data-cold-segmented-control-value="hidden" /> : null}
      {options.map(option => {
        ...
        return (
          <button
            ...
            data-cold-segmented-control-option={option.value}
            className={cn(
