...
const TagInput = React.forwardRef<HTMLInputElement, TagInputProps>(
  ({ className, containerClassName, disabled, name, value, onValueChange, placeholder, suggestions = [], maxSuggestions = 8, ...props }, ref) => {
    ...
    return (
      <div
        data-hz-tag-input-owner="hertzbeat-ui-tag-input"
        data-hz-tag-input-mode={suggestions.length > 0 ? 'searchable-tags' : 'freeform-tags'}
        className={cn(
      ...
      >
        <HiddenInput name={disabled ? undefined : name} value={value} data-hz-tag-input-value="hidden" />
        {tags.map(tag => (
          <span
            key={tag}
            data-hz-tag-chip={tag}
        ...
        <input
          ...
          disabled={disabled}
          value={draft}
          data-hz-tag-input-control="draft"
          className={cn(
        ...
        {focused && visibleSuggestions.length > 0 ? (
          <div
            data-hz-tag-suggestions-owner="hertzbeat-ui-search-popover"
            data-hz-tag-suggestions-position="fixed-anchored"
          ...
          >
            {visibleSuggestions.map(suggestion => (
              <button
                ...
                type="button"
                disabled={disabled}
                data-hz-tag-suggestion={suggestion}
