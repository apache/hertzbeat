...
public class OtlpLogController {
    ...
    public ResponseEntity<String> ingestJsonLogs(@RequestBody byte[] content, @RequestHeader HttpHeaders requestHeaders) {
        ...
        try {
            byte[] enrichedContent = otlpCorrelationEnricher.enrichLogsHttp(
                    content, requestHeaders, OtlpCorrelationContext.empty());
            greptimeOtlpForwarder.forwardLogsProtobuf(enrichedContent);
            otlpLogProtocolAdapter.publishRealtimeSignals(ExportLogsServiceRequest.parseFrom(enrichedContent));
            return ResponseEntity.ok(toJsonResponse(EMPTY_RESPONSE));
        } catch (StatusRuntimeException e) {
            return ResponseEntity.status(toHttpStatus(e))
                    .body(toJsonErrorResponse(e.getStatus().getDescription()));
    ...
    public ResponseEntity<byte[]> ingestBinaryLogs(@RequestBody byte[] content, @RequestHeader HttpHeaders requestHeaders) {
        ...
        try {
            ...
            return ResponseEntity.status(toHttpStatus(e))
                    .body(createBinaryErrorResponse(e.getStatus().getDescription()));
        } catch (IllegalArgumentException e) {
    ...
    private HttpStatus toHttpStatus(StatusRuntimeException exception) {
