...
export function useSessionIdentityBoundary() {
  const replaceIdentity = useContext(SessionIdentityContext);
  if (!replaceIdentity) throw new Error('Session identity boundary is missing.');
