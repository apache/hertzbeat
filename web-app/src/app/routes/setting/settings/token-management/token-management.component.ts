...
@Component({
  standalone: false,  selector: 'app-token-management',
...
export class TokenManagementComponent implements OnInit {
  ...
  get activeTokenCount(): number {
    return this.tokens.filter(token => !this.isExpired(token)).length;
  }
    ...
    this.authSvc.listTokens().subscribe(
      message => {
        if (message.code === 0) {
          ...
          this.loadErrorMessage = message.msg || this.i18nSvc.fanyi('settings.token.load-fail');
      ...
      () => {
        this.loadFailed = true;
        this.loadErrorMessage = this.i18nSvc.fanyi('settings.token.load-fail');
        this.loading = false;
      }
    );
  }
