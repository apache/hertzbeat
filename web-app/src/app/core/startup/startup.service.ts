...
export class StartupService {
  private readonly canonicalMenuLinks: Record<string, string> = {
    '/dashboard': '/overview',
    '/alert/center': '/alert'
  };
  ...
  public loadConfigResourceViaHttp(): Observable<void> {
    ...
    return this.resolveStartupLang().pipe(
      switchMap(defaultLang =>
        zip(
          ...
          map(([langData, appData, menuData]: [Record<string, string>, NzSafeAny, NzSafeAny]) => {
            ...
            this.menuService.add(this.normalizeMenuLinks(appData.menu));
  ...
  private resolveStartupLang(): Observable<string> {
    const defaultLang = this.i18n.defaultLang;
    return this.generalConfigService.getGeneralConfig('system').pipe(
      map(message => {
        const locale = message?.code === 0 ? message?.data?.locale : null;
        return locale ? String(locale).replace('_', '-') : defaultLang;
      }),
      catchError(() => [defaultLang])
    );
  }
  ...
  private normalizeMenuLinks(menus: Menu[] = []): Menu[] {
    return menus.map(menu => ({
      ...menu,
      link: menu.link ? this.canonicalMenuLinks[menu.link] ?? menu.link : menu.link,
      children: menu.children ? this.normalizeMenuLinks(menu.children as Menu[]) : menu.children
    }));
  }
}
