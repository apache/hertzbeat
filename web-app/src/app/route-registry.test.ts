...
describe('route registry', () => {
  ...
  it('preserves the master entry routes', () => {
    expect(routeRegistry.map(route => route.path)).toEqual(
      expect.arrayContaining(['/', '/dashboard', '/monitors', '/alerts', '/alerts/rules', '/alerts/groups', '/alerts/inhibits', '/alerts/silences', '/alerts/notifications/receivers', '/alerts/notifications/templates', '/bulletin', '/status', '/passport/login'])
    );
  });
});
