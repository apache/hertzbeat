...
describe('RouteTimeProvider', () => {
  it('derives route-owned exact time from canonical URL and converges on POP and refresh', async () => {
    const routed = renderTime([
      '/explore?signal=logs&start=1000&end=2000',
      '/explore?signal=traces&start=3000&end=4000'
    ], 1);
...
function renderTime(entries: string[], initialIndex = 0) {
  let value: ReturnType<typeof useSharedTime> | undefined;
  function Probe() { value = useSharedTime(); return null; }
  ...
  return {
    ...
    current: () => { if (!value) throw new Error('shared time not mounted'); return value; }
  };
}
