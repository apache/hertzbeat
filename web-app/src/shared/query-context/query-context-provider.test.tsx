...
describe('QueryContextProvider', () => {
  it('converges on Router POP and removes sensitive inbound URL fields', async () => {
    ...
    await waitFor(() => expect(routed.current().context).toMatchObject({ collectorId: 'west', serviceName: 'payments' }));
    await waitFor(() => expect(routed.router.state.location.search).not.toContain('token'));
    await act(async () => routed.router.navigate(-1));
    await waitFor(() => expect(routed.current().context).toMatchObject({ collectorId: 'east', serviceName: 'checkout' }));
    await act(async () => routed.router.navigate(1));
    await waitFor(() => expect(routed.current().context).toMatchObject({ collectorId: 'west', serviceName: 'payments' }));
...
function renderProvider(entries: string[], initialIndex = 0) {
  ...
  function Probe() { value = useQueryContext(); return null; }
  const router = createMemoryRouter([{
    path: '/explore', element: <QueryContextProvider><Probe /></QueryContextProvider>
  }], { initialEntries: entries, initialIndex });
  const view = render(<RouterProvider router={router} />);
  return {
    router,
    unmount: view.unmount,
    current: () => { if (!value) throw new Error('query context not mounted'); return value; }
  };
}
