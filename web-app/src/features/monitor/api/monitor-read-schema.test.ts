...
import { MonitorContractError, type MonitorQuery } from './monitor-contract';
...
const query: MonitorQuery = {
  search: '', app: '', status: '9', labels: '', pageIndex: 0, pageSize: 10
};
...
describe('Monitor primary read schemas', () => {
  ...
  it('enforces nested detail identities before returning mapped evidence', () => {
    expect(() => parseMonitorDetail({
      ...
      metrics: []
    }, 7)).toThrow(MonitorContractError);
  });

  it('maps only the app option fields used by monitor selection', () => {
    expect(parseMonitorApps([{
      category: 'http', value: 'website', label: 'Website', hide: false, children: ['ignored']
    }])).toEqual([{
