...
import type { MonitorHistoryChart } from '../model/monitor-detail-model';
import {
...
export function useMonitorInvestigation(monitor: Monitor | undefined, chart: MonitorHistoryChart | undefined) {
  const navigate = useNavigate();
  const query = useQuery({
    queryKey: monitorQueryKeys.investigation(monitor?.id),
    queryFn: monitor
      ? async ({ signal }) => (await loadMonitorInvestigationBinding(monitor.id, signal)) ?? null
      : skipToken,
    retry: false
  });
