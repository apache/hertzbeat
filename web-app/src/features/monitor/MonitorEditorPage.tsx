...
import { detectMonitor, loadMonitorApps, loadMonitorDetail, loadMonitorParamDefines, saveMonitor, type Monitor, type MonitorApp, type MonitorDetail, type MonitorParam, type MonitorParamDefine } from './monitor-api';
import { monitorAppOptions } from './monitor-model';
...
function MonitorParamInput({ define, param, label, update }: { define: MonitorParamDefine; param: MonitorParam | undefined; label: string; update: (value: unknown) => void }) {
...
function MonitorEditorForm({ mode, initial, defines, apps, changeApp }: { mode: 'new' | 'edit'; initial: MonitorDetail; defines: MonitorParamDefine[]; apps: MonitorApp[]; changeApp: (app: string) => void }) {
  ...
  const mutation = useMutation({ mutationFn: ({ action, payload }: { action: 'detect' | 'save'; payload: unknown }) => action === 'detect' ? detectMonitor(payload) : saveMonitor(mode, payload) });
  ...
  const submit = (action: 'detect' | 'save') => {
    ...
    mutation.mutate({ action, payload: buildMonitorPayload(monitor, collector, params) }, {
      onSuccess: () => {
        ...
        if (action === 'save') void navigate(`/monitors?app=${encodeURIComponent(monitor.app)}`);
      },
  ...
  return <>
    <div className={styles.form}>
      <label>{t('monitor.application')}<Select<string> disabled={mode === 'edit'} showSearch optionFilterProp="label" value={monitor.app || null} options={monitorAppOptions(apps)} onChange={app => { setMonitor(current => ({ ...current, app })); changeApp(app); }} /></label>
      <label>{t('monitor.name')}<Input value={monitor.name} onChange={event => setMonitor(current => ({ ...current, name: event.target.value }))} /></label>
      <label>{t('monitor.editor.interval')}<InputNumber<number> min={10} value={monitor.intervals ?? 60} onChange={value => setMonitor(current => ({ ...current, intervals: value ?? 60 }))} /></label>
      <label>{t('monitor.editor.collector')}<Input value={collector} placeholder={t('monitor.editor.collectorDefault')} onChange={event => setCollector(event.target.value)} /></label>
      <label className={styles.wide}>{t('monitor.editor.descriptionLabel')}<Input.TextArea rows={2} value={monitor.description} onChange={event => setMonitor(current => ({ ...current, description: event.target.value }))} /></label>
      {defines.length > 0 && <Typography.Title className={styles.section ?? ''} level={4}>{t('monitor.editor.connection')}</Typography.Title>}
      {defines.filter(define => !define.hide).map(define => <MonitorParamInput key={define.field} define={define} param={params.find(param => param.field === define.field)} label={paramLabel(define.name, define.field)} update={value => updateParam(define.field, value)} />)}
    </div>
    <div className={styles.actions}><Button onClick={() => void navigate('/monitors')}>{t('common.cancel')}</Button><Button loading={mutation.isPending} onClick={() => submit('detect')}>{t('monitor.editor.detect')}</Button><Button type="primary" loading={mutation.isPending} onClick={() => submit('save')}>{t('common.save')}</Button></div>
  </>;
}
...
export function MonitorEditorPage({ mode }: { mode: 'new' | 'edit' }) {
  ...
  const detail = useQuery({ queryKey: ['monitor-detail', monitorId], queryFn: () => loadMonitorDetail(monitorId), enabled: mode === 'edit' && Boolean(monitorId) });
  const activeApp = activeMonitorApp(mode, detail.data, selectedApp);
  const defines = useQuery({ queryKey: ['monitor-param-defines', activeApp], queryFn: () => loadMonitorParamDefines(activeApp), enabled: Boolean(activeApp) });
  ...
  return <div className={styles.page}>
    <header className={styles.heading}><Typography.Title level={2}>{t(editorTitleKey(mode))}</Typography.Title><Typography.Text type="secondary">{t('monitor.editor.description')}</Typography.Text></header>
    <EditorBody failed={failed} ready={ready}><MonitorEditorForm key={`${mode}:${monitorId}:${activeApp}`} mode={mode} initial={initial} defines={defines.data ?? []} apps={apps.data ?? []} changeApp={setSelectedApp} /></EditorBody>
  </div>;
}
