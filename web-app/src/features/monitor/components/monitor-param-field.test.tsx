...
const define = (field: string, type: string, patch: Partial<MonitorParamDefine> = {}): MonitorParamDefine => ({
  id: null, app: 'website', field, name: { 'en-US': field }, type, required: false, defaultValue: null,
  placeholder: null, range: null, limit: null, options: null, keyAlias: null, valueAlias: null,
  depend: null, hide: false, ...patch
});
const mapLabels = { add: 'Add pair', remove: 'Remove pair', key: 'Key', value: 'Value',
  emptyError: 'A key is required', duplicateError: 'Keys must be unique' };
...
describe('MonitorParamField', () => {
  it('masks passwords and renders authoritative radio options', () => {
    const password = render(<MonitorParamField define={define('token', 'password')} label="Token"
      value="secret" onChange={vi.fn()} mapLabels={mapLabels} metricsLabels={metricsLabels} />);
    expect(screen.getByLabelText('Token')).toHaveAttribute('type', 'password');
    password.unmount();

    render(<MonitorParamField define={define('mode', 'radio', { options: [
      { label: 'Basic', value: 'basic' }, { label: 'Token', value: 'token' }
    ] })} label="Mode" value="basic" onChange={vi.fn()} mapLabels={mapLabels} metricsLabels={metricsLabels} />);
  ...
  it('keeps array values comma-delimited and edits key-value objects structurally', () => {
    ...
    const view = render(<MonitorParamField define={define('codes', 'array')} label="Codes"
      value="200, 201" onChange={arrayChange} mapLabels={mapLabels} metricsLabels={metricsLabels} />);
    ...
    render(<MonitorParamField define={define('headers', 'key-value')} label="Headers"
      value="" onChange={mapChange} onValidityChange={validity}
      mapLabels={mapLabels} metricsLabels={metricsLabels} />);
  ...
  it('reports duplicate keys without overwriting the last valid map', () => {
    ...
    const validity = vi.fn();
    render(<MonitorParamField define={define('headers', 'key-value')} label="Headers"
      value={{ Accept: 'json' }} onChange={change} onValidityChange={validity}
      mapLabels={mapLabels} metricsLabels={metricsLabels} />);
    fireEvent.click(screen.getByRole('button', { name: 'Add pair' }));
  ...
  it('edits structured push metric rows and rejects duplicate fields', () => {
    const change = vi.fn();
    const validity = vi.fn();
    render(<MonitorParamField define={define('fields', 'metrics-field')} label="Metrics"
      value={[]} onChange={change} onValidityChange={validity} mapLabels={mapLabels} metricsLabels={metricsLabels} />);
    fireEvent.click(screen.getByRole('button', { name: 'Add pair' }));
  ...
  it('preserves existing metric label and i18n metadata while editing a visible field', () => {
    ...
    render(<MonitorParamField define={define('fields', 'metrics-field')} label="Metrics"
      ...
      onChange={change} mapLabels={mapLabels} metricsLabels={metricsLabels} />);
    fireEvent.change(screen.getByLabelText('Unit'), { target: { value: 'seconds' } });
    expect(change).toHaveBeenCalledWith([{
      field: 'latency', unit: 'seconds', type: 0, label: true, i18n: { 'en-US': 'Latency' }
    }]);
  });
