...
describe('Monitor editor payload', () => {
  ...
  it.each([
    ...
    const payload = buildMonitorPayload({ app: 'website', scrape: 'static', name: 'home', instance: 'ignored',
      status: 0 }, '', [
  ...
  it('uses the exact service-discovery sentinel and requires exact reread convergence', () => {
    const payload = buildMonitorPayload({ app: 'website', scrape: 'http_sd', name: 'home', instance: 'ignored',
      status: 0 }, '', [{ field: 'port', type: 0, paramValue: '443' }], [
  ...
  it('accepts only authoritative encrypted password convergence', () => {
    ...
    const monitor: Monitor = { id: 7, jobId: 9, app: 'website', name: 'home', instance: '', status: 0, type: 0,
      labels: {}, annotations: {}, scrape: 'static', intervals: 60, scheduleType: 'interval', cronExpression: null };
