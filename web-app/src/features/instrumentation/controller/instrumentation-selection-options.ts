...
export function buildInstrumentationSelectionOptions(
  ...
  const languages = catalog.languages.filter(language =>
    language.frameworks.some(
      framework => compatibleMethods(catalog, draft, language.language, framework.framework).length > 0
    )
  );
  const frameworks =
    selectedLanguage?.frameworks.filter(
      framework => compatibleMethods(catalog, draft, selectedLanguage.language, framework.framework).length > 0
    ) ?? [];
