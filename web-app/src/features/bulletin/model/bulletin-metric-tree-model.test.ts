...
import {
  BulletinMetricTreeError, buildBulletinMetricTree, fieldsFromMetricTreeKeys, resolveSavedMetricTreeSelection
} from './bulletin-metric-tree-model';

const hierarchy: MonitorAppHierarchyNode = {
  category: 'network', value: 'website', label: 'Website', isLeaf: false, hide: false, type: null, unit: null,
  children: [
    { category: null, value: 'summary', label: 'Summary', isLeaf: false, hide: null, type: null, unit: null,
      children: [
        { category: null, value: 'status', label: 'Status', isLeaf: true, hide: null, type: 0, unit: null, children: [] },
        { category: null, value: 'responseTime', label: 'Response time', isLeaf: true, hide: null,
          type: 0, unit: 'ms', children: [] }
      ] },
    { category: null, value: 'availability', label: 'Availability', isLeaf: false, hide: null,
      type: null, unit: null, children: [
        { category: null, value: 'status', label: 'Status', isLeaf: true, hide: null, type: 0, unit: null, children: [] }
      ] }
  ]
};
...
describe('bulletin metric tree model', () => {
  it('creates stable structural keys that do not collide for equal field names under different metrics', () => {
    ...
    expect(first.map(node => node.key)).toEqual([
      '["metric","summary"]', '["metric","availability"]'
    ]);
    ...
    expect(new Set(first.flatMap(node => [node.key, ...node.children.map(child => child.key)]))).toHaveProperty('size', 5);
  ...
  it('converts checked leaves only and canonicalizes select-all, partial, and uncheck-all payloads', () => {
    ...
    expect(fieldsFromMetricTreeKeys(tree, allKeys)).toEqual({
      availability: ['status'], summary: ['responseTime', 'status']
    });
    ...
    expect(fieldsFromMetricTreeKeys(tree, [tree[0]!.children[0]!.key, tree[0]!.children[0]!.key]))
      .toEqual({ summary: ['status'] });
  });

  it('backfills known saved leaves and reports unknown saved fields explicitly', () => {
    const tree = buildBulletinMetricTree(hierarchy);
    expect(resolveSavedMetricTreeSelection(tree, {
      summary: ['status', 'removed', 'status'], missingMetric: ['value']
    })).toEqual({
  ...
  it.each([
    ...
    { ...hierarchy, children: [{ ...hierarchy.children[0]!, children: [
      ...hierarchy.children[0]!.children, hierarchy.children[0]!.children[0]!
    ] }] },
    { ...hierarchy, children: [{ ...hierarchy.children[0]!, isLeaf: true }] },
    { ...hierarchy, children: [{ ...hierarchy.children[0]!, children: [{
      ...hierarchy.children[0]!.children[0]!, isLeaf: false
    }] }] }
  ...
  it('rejects checked keys that do not belong to the active hierarchy', () => {
    expect(() => fieldsFromMetricTreeKeys(buildBulletinMetricTree(hierarchy), ['["field","old","status"]']))
      .toThrow(BulletinMetricTreeError);
  });
});
