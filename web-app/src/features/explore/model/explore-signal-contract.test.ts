...
describe('Explore signal contracts', () => {
  it('strips unknown metric fields while retaining explicit console evidence', () => {
    expect(parseMetricConsole({
      context: null, query: 'up', datasource: 'prometheus', queryMode: 'manual', unknown: 'drop',
      results: { refId: 'A', status: 200, msg: null, frames: [{ schema: { fields: [{ name: 'value', type: 'number', unit: null, extra: true }], labels: {}, meta: {} }, data: [[1, 2]], extra: true }] },
      stats: { totalSeries: 1, nonEmptySeries: 1, latestObservedAt: 2 }, emptyStateReason: null, errorMessage: null
    })).toEqual({
      context: null, query: 'up', datasource: 'prometheus', queryMode: 'manual',
      results: { refId: 'A', status: 200, msg: null, frames: [{ schema: { fields: [{ name: 'value', type: 'number', unit: null }], labels: {}, meta: {} }, data: [[1, 2]] }] },
      stats: { totalSeries: 1, nonEmptySeries: 1, latestObservedAt: 2 }, emptyStateReason: null, errorMessage: null
    });
  });
  ...
  it('retains JSON-safe log body and attributes and strips unknown fields', () => {
    const page = parseLogPage(springPage([{
      timeUnixNano: 10, observedTimeUnixNano: null, severityNumber: 9, severityText: 'INFO',
      body: { event: ['paid', 1, true, null] }, attributes: { nested: { ok: true } }, droppedAttributesCount: 0,
      traceId: null, spanId: null, traceFlags: null, resource: {}, resourceSchemaUrl: null,
      instrumentationScope: null, scopeSchemaUrl: null, secret: 'drop'
    }]), 0, 20);
  ...
  it('accepts the lossy Java Long number representation used for epoch nanoseconds', () => {
    ...
    expect(parseLogPage(springPage([{
      timeUnixNano: epochNanos, observedTimeUnixNano: epochNanos, severityNumber: null, severityText: null,
      body: null, attributes: null, droppedAttributesCount: null, traceId: null, spanId: null, traceFlags: null,
      resource: null, resourceSchemaUrl: null, instrumentationScope: null, scopeSchemaUrl: null
  ...
  it('rejects content beyond the authoritative last-page remainder', () => {
    const content = Array.from({ length: 2 }, () => ({
      timeUnixNano: null, observedTimeUnixNano: null, severityNumber: null, severityText: null, body: null,
      attributes: null, droppedAttributesCount: null, traceId: null, spanId: null, traceFlags: null,
      resource: null, resourceSchemaUrl: null, instrumentationScope: null, scopeSchemaUrl: null
    }));
    expect(() => parseLogPage({ content, totalElements: 21, totalPages: 2, number: 1, size: 20 }, 1, 20))
      .toThrow(/content/);
  });
  ...
  it('keeps authoritative empty and out-of-range pages distinct', () => {
    ...
    expect(parseLogPage({ ...springPage([]), number: 3, totalElements: 1, totalPages: 1 }, 3, 20))
      .toMatchObject({ content: [], totalElements: 1, number: 3 });
  });
  ...
  it('parses nested trace evidence with allowlists', () => {
    const detail = parseTraceDetail({ ...traceRow('trace-1'), unknown: true, spans: [{
      traceId: 'trace-1', spanId: 'span-1', parentSpanId: null, spanName: 'GET', serviceName: 'checkout',
      status: 'OK', spanKind: 'SERVER', statusMessage: null, traceState: null, scopeName: null, scopeVersion: null,
      durationNanos: 2, startTime: 1, highlighted: false, resourceAttributes: {}, spanAttributes: {},
      events: [{ timeUnixNano: 1, name: 'event', attributes: { value: 1 }, droppedAttributesCount: 0, extra: true }],
      links: [], codeNavigationHint: null, extra: true
    }] }, 'trace-1');
  ...
  it.each([
    ...
    [[{ spanId: 'span-1', traceId: 'trace-1' }, { spanId: 'span-1', traceId: 'trace-1' }]]
  ])('rejects invalid canonical span identity %s', identities => {
    const spans = identities.map(identity => ({ ...identity, parentSpanId: null, spanName: null, serviceName: null,
      status: null, spanKind: null, statusMessage: null, traceState: null, scopeName: null, scopeVersion: null,
      durationNanos: null, startTime: null, highlighted: false, resourceAttributes: null, spanAttributes: null,
      events: null, links: null, codeNavigationHint: null }));
