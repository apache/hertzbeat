...
import { requireExactAlertGroupId } from '../alert-group-write-proof';
...
export function useAlertGroupCommandGate() {
  ...
  return { command, begin, end, isLocked: () => commandRef.current !== 'idle' };
}
  ...
  const edit = (id: number): Promise<void> => {
    ...
    const promise = (async () => {
      try {
        ...
        if (detailEpochRef.current === epoch) setDetail({ kind: classifyAlertGroupReadError(reason), id });
      } finally {
...
export function useAlertGroupEditor(gate: AlertGroupCommandGate) {
  ...
  return {
    ...
    actions: { create, edit: detailController.edit, retryDetail: detailController.retryDetail, closeDraft, updateDraft }
  };
}
