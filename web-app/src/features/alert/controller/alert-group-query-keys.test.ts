...
describe('Alert Group Query Keys', () => {
  it('includes every backend list input in stable order', () => {
    expect(alertGroupQueryKeys.list(query)).toEqual([
      'alert-group-policies', 'list', 'service', 2, 15
    ]);
  });
  ...
  ] satisfies Array<[string, Partial<AlertGroupQuery>]>)('separates cache evidence when %s changes', (_field, patch) => {
