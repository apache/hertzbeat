...
describe('notice receiver model', () => {
  ...
  it('retains, replaces, and explicitly clears only active-type secrets', () => {
    const evidence = { id: 7, name: 'Incident gateway', type: 2 as const, typeKey: 'webhook',
      options: { hookAuthType: 'Bearer' as const }, configuredSecrets: ['hookUrl', 'hookAuthToken'] as const };
    const retained = noticeReceiverDraftFromDetail(evidence);
    expect(buildNoticeReceiverPayload(retained)).toEqual({
      id: 7, name: 'Incident gateway', type: 2, options: { hookAuthType: 'Bearer' }
    });

    const replaced = { ...retained, hookAuthToken: 'replacement' };
    expect(buildNoticeReceiverPayload(replaced).options).toEqual({ hookAuthType: 'Bearer', hookAuthToken: 'replacement' });

    const cleared = setNoticeReceiverSecretCleared(replaced, 'hookAuthToken', true);
    expect(cleared.hookAuthToken).toBe('');
    expect(buildNoticeReceiverPayload(cleared).options).toEqual({ hookAuthType: 'Bearer', clearSecrets: ['hookAuthToken'] });
  });

  it('clears secret ownership and stale options when the receiver type changes', () => {
    const draft = { ...createNoticeReceiverDraft(), type: 2 as const, name: 'Gateway',
      hookUrl: 'replacement', hookAuthToken: 'token', configuredSecrets: ['hookUrl', 'hookAuthToken'] as const,
    ...
    expect(buildNoticeReceiverPayload({ ...selected, email: 'ops@example.test' })).toEqual({
      name: 'Gateway', type: 1, options: { email: 'ops@example.test' }
    });
  });

  it('matches frozen required fields for WeCom, DingTalk, and FeiShu', () => {
    const wecom = { ...createNoticeReceiverDraft(), name: 'WeCom', type: 10 as const,
      corpId: 'corp', agentId: 1, appSecret: 'secret', userId: '' };
    ...
    const feishu = { ...createNoticeReceiverDraft(), name: 'FeiShu', type: 14 as const,
      appId: 'app', appSecret: 'secret', larkReceiveType: 1 as const };
  ...
  it('does not allow clearing a required configured secret', () => {
    const draft = noticeReceiverDraftFromDetail({ id: 8, name: 'Slack', type: 8, typeKey: 'slack-webhook',
      options: {}, configuredSecrets: ['slackWebHookUrl'] });
    expect(validateNoticeReceiverDraft(setNoticeReceiverSecretCleared(draft, 'slackWebHookUrl', true)))
      .toContain('slackWebHookUrl');
  });

  it('derives canonical public options and secret names without exposing secret values', () => {
    const draft = noticeReceiverDraftFromDetail({ id: 7, name: 'Gateway', type: 2, typeKey: 'webhook',
      options: { hookAuthType: 'Bearer' }, configuredSecrets: ['hookUrl', 'hookAuthToken'] });
    const changed = { ...draft, hookUrl: 'replacement-url', clearSecrets: ['hookAuthToken'] as const };
    const expected = expectedNoticeReceiverEvidence(changed);
    expect(expected).toEqual({ options: { hookAuthType: 'Bearer' }, configuredSecrets: ['hookUrl'] });
  ...
  it('does not expose secret-bearing channel settings in list summaries', () => {
    expect(noticeReceiverSettingSummary({ id: 1, name: 'Slack', type: 8, typeKey: 'slack-webhook', options: {},
      configuredSecrets: ['slackWebHookUrl'] }))
      .toEqual({ kind: 'configured' });
    expect(noticeReceiverSettingSummary({ id: 2, name: 'Email', type: 1, typeKey: 'email',
      options: { email: 'ops@example.test' }, configuredSecrets: [] }))
