...
describe('notice receiver API contract', () => {
  ...
  it('parses safe detail, page, and minimal rule options from exact endpoints', async () => {
    http.apiMessageGet.mockResolvedValueOnce(safeReceiver).mockResolvedValueOnce({
      content: [safeReceiver], totalElements: 1, totalPages: 1, number: 0, size: 8
    ...
    await expect(loadNoticeReceivers({ name: '', pageIndex: 0, pageSize: 8 })).resolves.toMatchObject({
      content: [safeReceiver], totalElements: 1
    });
  ...
  it('sends structured active-type payloads and validates mutation evidence', async () => {
    http.apiMessagePost.mockResolvedValueOnce({ id: 7, status: 'created', receiver: safeReceiver })
    ...
    const create = { ...createNoticeReceiverDraft(), name: 'Pager', type: 2 as const,
      hookUrl: 'https://secret.test/hook', hookAuthType: 'Bearer' as const, hookAuthToken: 'token' };
    const update = { ...create, id: 7, hookUrl: '', hookAuthToken: '', configuredSecrets: ['hookUrl', 'hookAuthToken'] as const };
    ...
    expect(http.apiMessagePost).toHaveBeenNthCalledWith(1, '/api/notice/receiver', {
      name: 'Pager', type: 2, options: { hookUrl: 'https://secret.test/hook', hookAuthType: 'Bearer', hookAuthToken: 'token' }
    });
    expect(http.apiMessagePut).toHaveBeenCalledWith('/api/notice/receiver', {
      id: 7, name: 'Pager', type: 2, options: { hookAuthType: 'Bearer' }
    });
    expect(http.apiMessagePost).toHaveBeenNthCalledWith(2, '/api/notice/receiver/send-test-msg', expect.objectContaining({
  ...
  it('rejects invalid successful mutation evidence', async () => {
    http.apiMessagePost.mockResolvedValue({ id: 7, status: 'created', receiver: { ...safeReceiver, options: { hookUrl: 'echo' } } });
