...
vi.mock('./notice-rule-query-controller', () => ({
  useNoticeRuleQueryController: () => ({ query: { name: '', pageIndex: 0, pageSize: 8 }, name: '',
    setName: vi.fn(), search: vi.fn(), changePage: vi.fn() })
}));
...
describe('notice rule controller', () => {
  beforeEach(() => {
    ...
    mocks.useDataProvider.mockReturnValue(() => ({ create: mocks.create, deleteOne: mocks.deleteOne,
      getOne: mocks.getOne, update: mocks.update }));
    mocks.useNotification.mockReturnValue({ open: mocks.notification });
    mocks.useList.mockReturnValue({ query: { error: null, isError: false, isFetching: false,
      isPending: false, refetch: mocks.refetch }, result: { data: [], total: 0 } });
  });
  ...
  it('keeps the editor open and preserves unavailable classification when plain refetch evidence fails', async () => {
    ...
    expect(mocks.notification).toHaveBeenCalledWith(expect.objectContaining({ message: 'noticeRules.save.unavailable' }));
  });
  ...
  it('persists a list switch change and reports success only after false detail convergence and list reread', async () => {
    ...
    expect(mocks.notification).toHaveBeenCalledWith(expect.objectContaining({
      message: 'noticeRules.saveSuccess', type: 'success'
    }));
  });
  ...
  it('gives a post-rename toggle its own observable per-row command boundary', async () => {
    ...
    mocks.update.mockImplementationOnce(() => new Promise(resolve => { resolveToggle = resolve; }));
    let togglePromise: Promise<boolean> | undefined;
    act(() => { togglePromise = result.current.actions.toggle({ ...rule, name: 'Renamed' }, false); });
    ...
    expect(mocks.update).toHaveBeenLastCalledWith(expect.objectContaining({
      id: 31, variables: expect.objectContaining({ draft: expect.objectContaining({ enable: false }) })
    }));
  });
});
