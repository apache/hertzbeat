...
import { compatibleNoticeRuleTemplates, noticeRuleWeekdays, type NoticeRuleDraft } from "../model/notice-rule-model";
...
function NoticeRuleAdvancedFields({
  draft,
  update,
}: {
  ...
  return (
    <div className={styles.advancedFields}>
      ...
      <label className={styles.switchField}>
        ...
        <Switch
          checked={draft.limitDays}
          onChange={(limitDays) => update({ limitDays, ...(limitDays ? {} : { days: [1, 2, 3, 4, 5, 6, 7] }) })}
        />
      </label>
      {draft.limitDays && (
        <label className={styles.wideField}>
