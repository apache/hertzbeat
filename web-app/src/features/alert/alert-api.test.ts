...
const query = {
  search: '', status: '', severity: '', serviceName: 'checkout', serviceNamespace: 'payments',
  environment: 'prod', pageIndex: 0, pageSize: 8
} as const;
...
describe('alert API', () => {
  ...
  it('parses and allowlists summary data on read', async () => {
    apiMessageGet.mockResolvedValue({
      total: 2, dealNum: 1, rate: 50, priorityWarningNum: 1,
      priorityCriticalNum: 0, priorityEmergencyNum: 0, leaked: 'ignored'
    });
